...
const IconSidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  ({ activeMenu, setActiveMenu }, ref) => {
    ...
    return (
      <aside
        ref={ref}
        className="w-20 z-[999] h-full flex flex-col rounded-r-[20px] justify-center items-center bg-sidebar text-sidebar-foreground"
      >
              ...
              <div
                ...
                className={`
                    ...
                    ${activeMenu === id && (isContentSidebarVisible || isPinned)
                      ? 'bg-sidebar-ring text-primary-foreground'
                      : 'hover:bg-muted'}
                  `}
              >
                {item.icon}
