...
const ContentSidebar = React.forwardRef<HTMLDivElement, ContentSidebarProps>(
  ({ setSelectedMenu }, ref) => {
    ...
    const [submenuItems, setSubmenuItems] = useState<Array<{
      href: string;
      label: string;
    }> | null>(null);
      ...
      <>
                    ...
                    {content.links.map((link) => (
                      <li key={link.href} className="relative">
                        <div
                          onMouseEnter={(e) => {
                            if (link.submenu) {
                              openSubmenu(e, link.href, link.submenu); // always open new submenu
                            } else {
                              closeSubmenu(); // close submenu if this link has no submenu
                            }
                          }}
                        >
                          <Link
        ...
        {hoveredLink && submenuPos && submenuItems && (
          ...
          >
            ...
            <nav className="p-3 max-h-[calc(100vh-16px)] overflow-y-auto">
              <ul>
